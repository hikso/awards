...
const jsonToDataModel = () => {
    ...
    data.items.forEach(cat => {
        const _category = new Category(cat.title);
        cat.items.forEach(item => {
            const _nominee = new Nominee(item.photoUrL, item.title);
            _category.addNominee(_nominee);
        });
        categories.addCategory(_category);
    });
}
